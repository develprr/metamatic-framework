...
const handle = (listenerId, eventId, handler ) => {
...
export const attach = (component, eventId, handler) =>
  handle(component.constructor.name, eventId, handler);

export const attachAll = (component, handlerMap) => {
  const listenerId = component.constructor.name;
  for (let eventId in handlerMap) {
    let handler = handlerMap[eventId];
    handle(listenerId, eventId, handler);
  }
};

export const detach = (component) =>
